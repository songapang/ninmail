...
Ninmail is a Python library designed to streamline the process of creating email accounts across various top email provider services. With Ninmail, you can automate the creation of email accounts, saving time and effort. It provides an easy-to-use interface for creating accounts with customizable options.

## Features

- **Automated Account Creation:** Ninmail streamlines the process of creating email accounts by automating the necessary steps.
- **Support for Major Email Providers:** Ninmail supports a wide range of popular email service providers including Gmail, Outlook and Yahoo, giving you flexibility in choosing the provider that suits your needs.
- **Python Integration:** Ninmail seamlessly integrates into Python projects, allowing for efficient automation of email account creation.
...
- **Error Handling and Logging:** Ninmail provides error handling capabilities and logs activities to facilitate debugging and tracking of account creation actions.
- **Open-Source and Extensible:** Being an open-source project, Ninmail encourages contributions and allows for further extension and improvement of its functionalities.
- **Proxy Support:** Ninmail includes proxy support, giving users the option to use their own proxies for account creation, including support for authenticated proxies. This feature allows for enhanced privacy, security, and flexibility during the email account creation process.
- **Free Proxy Option:** Additionally, Ninmail offers an option to automatically retrieve and use free proxies. This feature provides users with a convenient solution for proxy usage, eliminating the need for purchasing or configuring proxies separately.

## Installation

To install Ninmail, you can follow these steps:

1. Clone the Ninmail repository from GitHub using the following command:

   Copy

   ```bash
   git clone https://github.com/songapang/ninmail.git
   ```
...
You can then proceed to use Ninmail as described in the next instructions.
...
To use Ninmail in your Python script, import the `Ninmail` class from the `Ninmail` module:

```python
from Ninmail import Ninmail
```

### Initializing Ninmail

To create an instance of Ninmail, call the `Ninmail` class with optional parameters:
...
The `browser` parameter specifies the browser to be used for automation. The default value is "firefox". Currently, Ninmail supports **Firefox, Chrome and Undetected Chrome**. The acceptable values for the browser parameter are `firefox`, `chrome` and `undetected-chrome` respectively.
...
The `auto_proxy` parameter is a boolean flag that determines whether Ninmail should automatically obtain and rotate free proxies during automation tasks. If `auto_proxy` is set to `True`, Ninmail will handle the process of acquiring and managing free proxies internally.

Please note that when `auto_proxy` is enabled, Ninmail will handle the management of proxies, but the availability and reliability of free proxies may vary. It's important to consider the limitations and potential risks associated with using free proxy services.

### Creating Outlook Accounts

To create an Outlook/Hotmail account using Ninmail, call the `create_outlook_account` method:
...
To create a Gmail account using Ninmail, call the `create_gmail_account` method:
...
To create a Yahoo account using Ninmail, call the `create_yahoo_account` method:
...
Ninmail logs its activities to a file named `Ninmail.log` in the `logs` directory. The log file has a format of `[timestamp] [log_level]: log_message`. The log levels are: DEBUG, INFO, WARNING, ERROR, and CRITICAL.

## Example

Here's an example that shows how to use Ninmail to create an Outlook account with `undetected-chrome`:
...
Here's an example that demonstrates how to use Ninmail to create a Yahoo account without providing any user information. Ninmail will generate all the necessary data for you, including name, birthdate, etc. This example utilizes smspool as the SMS service, chrome as the web browser and an authenticated proxy:
...
Ninmail currently supports account creation for the following email providers:
...
Ninmail currently supports three SMS services providers for phone verification during account creation:

**getsmscode.com**

**Required Data:**

To use getsmscode.com with Ninmail, you'll need to acquire the following information:

- **Username:** Your getsmscode.com username.
- **Token:** Your API token from getsmscode.com.

**Using getsmscode.com with Ninmail:**

1. Include the `sms_keys` argument when initializing the Ninmail object:

   ```python
   ninja = Ninmail(sms_keys={"getsmscode": {"user": "YOUR_USERNAME", 
                              		"token": "YOUR_TOKEN"}})
   ```
...
To use smspool with Ninmail, you'll need to acquire the following information:

- **Token:** Your API token from smspool.

**Using smspool.net with Ninmail:**

1. Include the `sms_keys` argument when initializing the Ninmail object:

   ```python
   ninja = Ninmail(sms_keys={"smspool": {"token": "YOUR_TOKEN"}})
   ```
...
To use 5sim with Ninmail, you'll need to acquire the following information:

- **Token:** Your API token from 5sim.

**Using 5sim with Ninmail:**

1. Include the `sms_keys` argument when initializing the Ninmail object:

   ```python
   ninja = Ninmail(sms_keys={"5sim": {"token": "YOUR_TOKEN"}})
   ```
...
Contributions are welcome! If you have ideas for new features, encounter issues, or want to improve Ninmail, feel free to contribute by opening issues and pull requests.

## Community

Join the Ninmail community to connect with other users, ask questions, and get support. We have a Telegram group where you can actively participate in discussions related to the library.

- Telegram Group: Join the Ninmail Community on Telegram [here](https://t.me/Ninmail).
...
Ninmail is provided solely for educational and informational purposes. It is explicitly stated that the intention behind Ninmail is not to promote or engage in any illegal activities, including hacking or any other unauthorized actions.

While using Ninmail, it is crucial to understand and abide by the terms of service of each email provider you choose to utilize. Creating accounts that violate these terms of service may lead to the suspension or termination of your account by the respective email provider.

To ensure responsible and lawful usage of Ninmail, please consider the following additional points:

1. Unethical Use: Under no circumstances should Ninmail be employed for malicious or unethical activities, including but not limited to spamming, phishing, or identity theft. Such actions are strictly prohibited.
2. Legal Compliance: You are solely responsible for ensuring that your use of Ninmail is in full compliance with all applicable laws and regulations within your jurisdiction. Any misuse that violates the law is strictly discouraged.

By choosing to use Ninmail, you acknowledge and accept the aforementioned disclaimers and agree to utilize this service only for educational and lawful purposes. Any misuse or illegal activities conducted using Ninmail are entirely the responsibility of the user, and the developers and providers of Ninmail bear no liability for such actions.
